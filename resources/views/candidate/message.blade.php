...
?>  
  ...
  <div class="utf-dashboard-content-container-aera" data-simplebar>
    ...
    <div class="utf-dashboard-content-inner-aera"> 
      <?php if(count($getMessageThread) > 0 || !empty($receiver)){ ?>
          ...
          <div class="message-person-list"> 
              <div class="content">
                <div class="messages">
                  ...
                  ?>
                            <div class="message-item" data-messageid="{{$vs->convHeadId}}" data-company="{{$receiverId}}">
            ...
            <div class="message-trail-container"> 
                ...
                <div class="message-input-container">
                  ...
                  <input type="hidden" name="hdnFromrecp" id="hdnFromrecp" value="{{session('candidate_session_data.userId')}}">
                  ...
                  <a href="javascript:void(0);" onclick="return submitchat();" class="button"><span class="icon-feather-send"></span></a>      
                </div>
              </div>        
            </div>

            <?php }else if(empty($receiver)){ ?>
...
>
  $(document).ready(function(){
    ...
    loadnotifiation(userid,2);
     ...
     $('input[type=file]').change(function(e) { 
        ...
        $.ajax({
          url: SITE_URL + "/candidate/ajax/uploadTempChatFile",
          type: 'POST',
    ...
    if(chklength > 0){
      ...
      var existingcom = $(this).data('company');
      var msgId       = $(this).data('messageid');
      //alert(existingcom);
      compArr.push(existingcom);
      if(existingcom == {{$receiver}}){
        $('.message-item').removeClass('active');
        $(this).addClass('active');
        loadMessagehistory(msgId,{{session('candidate_session_data.userId')}},existingcom);
        return false;
      }
      });
      //console.log($.inArray({{$receiver}}, compArr));
      if($.inArray({{$receiver}}, compArr) < 0){
        initiatechathead({{session('candidate_session_data.userId')}},{{$receiver}});
        return false;
      }
    }else{
      initiatechathead({{session('candidate_session_data.userId')}},{{$receiver}});
  ...
  $('.message-item').click(function(){
    ...
    $(this).addClass('active');
    loadMessagehistory(messageid,{{session('candidate_session_data.userId')}},companyid);
  })
</script>
@endsection
@endsection
