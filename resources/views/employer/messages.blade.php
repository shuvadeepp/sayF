...
<div class="utf-dashboard-content-container-aera" data-simplebar>
    ...
    <div class="utf-dashboard-content-inner-aera"> 
      ...
      <?php if(count($getMessageThread) > 0 || !empty($reqCandidateId)){ ?>
          ...
          <div class="message-person-list"> 
                    ...
                    foreach ($getMessageThread as $ks => $vs) {
                        ...
                        if($vs->respOne != session('employer_session_data.userId')){
                          if($vs->sender->tinUserType == 4){
                            ...
                            if(file_exists($partnerlogodirpath.'/'.$img)){
                            ...
                            if(file_exists($candidatelogodirpath.'/'.$img)){
            ...
            <div class="message-trail-container"> 
                ...
                <div class="message-input-container">
                  ...
                  <input type="hidden" name="hdnFromrecp" id="hdnFromrecp" value="{{session('employer_session_data.userId')}}">
      ...
      <?php }else if(empty($reqCandidateId)){ ?>  
        <!-- Section to show when there is no message available -->
            <div class="no-message">
              <div class="no-message__card">
                ...
                <p>No worries, <a href="<?php echo ROOT_URL.'/employer/job/managejob'?>" title="">Explore Applied Candidates</a></p>
              </div>
            </div>
      <?php } ?>  
...
>
  ...
  $(function(){ 
    ...
    $('input[type=file]').change(function(e) { 
        ...
        $.ajax({
          url: SITE_URL + "/employer/messages/uploadTempChatFile",
          type: 'POST',
    ...
    <?php if(!empty($reqCandidateId)){ ?>
      ...
      if(chklength > 0){
        ...
        $('.message-item').each(function(){
          ...
          if(existinguser == {{$reqCandidateId}}){
            $('.message-item').removeClass('active');
            $(this).addClass('active');
            loadMessageshistory(msgId,<?php echo session('employer_session_data.userId') ?>,existinguser);
            return false;
          }
        });
        if($.inArray({{$reqCandidateId}}, compArr) < 0){
          initiateChat(<?php echo session('employer_session_data.userId') ?>,<?php echo $reqCandidateId; ?>);
          return false;
        }
      }else{
        initiateChat(<?php echo session('employer_session_data.userId') ?>,<?php echo $reqCandidateId; ?>);
  ...
  $('.message-item').click(function(){
    ...
    $(this).addClass('active');
    loadMessageshistory(messageid,<?php echo session('employer_session_data.userId'); ?>,candidateid);
  });
  ...
  function loadMessageshistory(msgHead,userId,candidateId){
    ...
    $.ajax({
      type        : 'POST',
      url         : SITE_URL + "/employer/messages/loadMessagehistory",
  ...
  function sendMessage(){
    ...
    if(hdnFromrecp > 0 && hdnTorecp > 0 && msgarea != ''){
      $.ajax({
        type        : 'POST',
        url         : SITE_URL + "/employer/messages/sendMessage",
  ...
  if(senderId > 0 && receiverId > 0){
    $.ajax({
      type        : 'POST',
      url         : SITE_URL + "/employer/messages/initiateChat",
