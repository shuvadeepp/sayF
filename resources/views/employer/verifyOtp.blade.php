    ...
    <div class="row">
      ...
      <div class="col-xl-6 offset-xl-3">
        <div class="utf-login-register-page-aera margin-bottom-50"> 
          ...
          <form method="post" id="reset-form" autocomplete="off">
            {{csrf_field()}}
              <div class="otp-container">
                <div class="otp-box">
                  <input type="text" maxlength="4" class="otp-input-field" maxlength="32" name="otpemployeer" id="otpemployeer" placeholder="">
                   <span class="errMsg_otpemployeer errDiv"></span>
                </div>
              </div>
          ...
          <input type="hidden" value="<?php echo  !empty($getEmployeerData[0]['userId'])?$getEmployeerData[0]['userId']:'';?>" id="userid">
        </div>
      </div>
       <?php }?>
    </div>
...
<script>
  ...
  $(document ).ready(function() {
    ...
    if(status==1){
      ...
      $('.emp').magnificPopup().magnificPopup('open');
    }
  ...
  // OTP Field
  var obj = document.getElementById('otpemployeer');
  ...
  function validator()
  {
      ...
      $('.error-input').removeClass('error-input');
       if (!blankCheck('otpemployeer', 'OTP can not be left blank'))
          return false;
      $('#reset-form').submit();  
