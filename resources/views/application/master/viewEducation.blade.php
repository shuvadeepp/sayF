...
@section('page-content')
  ...
  <div class="utf-dashboard-content-container-aera" data-simplebar>
    <div id="dashboard-titlebar" class="utf-dashboard-headline-item">
      <div class="row">
        <div class="col-xl-12"> 
          <h3>View Candidate Education</h3>
          <nav id="breadcrumbs">
            <ul>
              <li><a href="{{ROOT_URL}}/application/dashboard">Home</a></li>
              <li>Manage Master</li>
              <li>View Education</li>
            </ul>
          </nav>
        </div>
      </div>    
    ...
    <div class="utf-dashboard-content-inner-aera"> 
      <div class="row"> 
        <div class="col-xl-12">
          <div class="mb-2">
            <a href="<?php echo ROOT_URL.'/application/master/education/add/'?>" class="button utf-ripple-effect-dark">Add Candidate Education</a>      
          </div>
        </div>
      </div>
...
Sl No.</th>
          ...
          <tbody>
            ...
            foreach ($arrAllRecords as $row){ 
              $eduTypeval=json_decode(EDUCATION_TYPE,true);?>
            <tr <?php if($row->publishStatus==1){ ?>class="unpublish" <?php }else {?>class="publish"<?php } ?>>
              <td><input type="checkbox" name="chkItem[]" class="mb-0 chkItem" value="{{$row->educationId}}"></td>
              <td>{{$ctr}}</td>
              <td>{{$row->educationName}}</td>
              <td>{{$eduTypeval[$row->educationType]}}</td>
              <td class="text-center"><a href="<?php echo ROOT_URL.'/application/master/education/add/'.encrypt($row->educationId)?>" class="btn btn-primary btn-xs" style="position: relative;top: 4px;">Edit</a></td>
            </tr>
          <?php $ctr++ ;} ?>
          </tbody>
          </table>
