  ...
  <div class="utf-dashboard-content-container-aera" data-simplebar>
    ...
    <div class="utf-dashboard-content-inner-aera"> 
      <div class="row"> 
        <div class="col-xl-12">
          <div class="mb-2">
            <a href="<?php echo ROOT_URL.'/application/master/skill/add/'?>" class="button utf-ripple-effect-dark">Add Skill</a>      
          </div>
        </div>
      </div>
...
Sl No.</th>
          ...
          <tbody>
            ...
            <tr  <?php if($row->publishStatus==1){ ?>class="unpublish" <?php }else {?>class="publish"<?php } ?>>
              <td><input type="checkbox" name="chkItem[]" class="mb-0 chkItem" value="{{$row->skillsId}}"></td>
              <td>{{$ctr}}</td>
              <td>{{$row->designationName}}</td>
              <td>{{$row->skillName}}</td>
              <td class="text-center"><a href="<?php echo ROOT_URL.'/application/master/skill/add/'.encrypt($row->skillsId)?>" class="btn btn-primary btn-xs" style="position: relative;top: 4px;">Edit</a></td>
            </tr>
          <?php $ctr++ ;} ?>
          </tbody>
          </table>
