  ...
  <div class="utf-dashboard-content-container-aera" data-simplebar>
    ...
    <div class="utf-dashboard-content-inner-aera"> 
      <div class="row"> 
        <div class="col-xl-12">
          <div class="mb-2">
            <a href="<?php echo ROOT_URL.'/application/master/blog/add/'?>" class="button utf-ripple-effect-dark">Add Blog</a>      
          </div>
        </div>
      </div>
...
Sl No.</th>
          ...
          <tbody>
            ...
            <tr <?php if($row->publishStatus==1){ ?>class="unpublish" <?php }else {?>class="publish"<?php } ?>>
              <td><input type="checkbox" name="chkItem[]" class="mb-0 chkItem" value="{{$row->blogId}}"></td>
              <td>{{$ctr}}</td>
              <td>{{$row->blogTitle}}</td>
              <td>{{date('d M Y',strtotime($row->publishDate))}}</td>
              <td>{{$row->postedBy}}</td>
              <td class="text-center"><a href="<?php echo ROOT_URL.'/application/master/blog/add/'.encrypt($row->blogId)?>" class="btn btn-primary btn-xs" style="position: relative;top: 4px;">Edit</a></td>
