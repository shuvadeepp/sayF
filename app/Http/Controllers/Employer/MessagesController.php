...
class MessagesController extends AppController {
    public function index($reqCandidateId = ''){
      ...
      $this->viewVars['reqCandidateId'] = ($reqCandidateId)?decrypt($reqCandidateId):0;                          
      return view('employer.messages',$this->viewVars);      
    }
            ...
            if($getAllmessage->isNotEmpty()){
              foreach ($getAllmessage as $ks => $vs) {
                if($vs['msgFrom'] != $userId){
                  ...
                  if($vs->senderdtls->tinUserType == 4){
                    $name          = ($vs->senderdtls->partner->partnerName)?$vs->senderdtls->partner->partnerName:'';
                  }else if($vs->senderdtls->tinUserType == 3){
                    if($vs->senderdtls->candidate->middleName != ''){
                      $name        = $vs->senderdtls->candidate->firstName.' '.$vs->senderdtls->candidate->middleName.' '.$vs->senderdtls->candidate->lastName;
                    }else{
                      $name        = $vs->senderdtls->candidate->firstName.' '.$vs->senderdtls->candidate->lastName;
                    }
                    //$name          = ($vs->senderdtls->candidate->firstName)?$vs->senderdtls->candidate->firstName:'';
                  }
                }else{
                  if($vs->receiverdtls->tinUserType == 4){
                    $name          = ($vs->receiverdtls->partner->partnerName)?$vs->receiverdtls->partner->partnerName:'';
                  }else if($vs->receiverdtls->tinUserType == 3){
                    if($vs->receiverdtls->candidate->middleName != ''){
                      $name        = $vs->receiverdtls->candidate->firstName.' '.$vs->receiverdtls->candidate->middleName.' '.$vs->receiverdtls->candidate->lastName;
                    }else{
                      $name        = $vs->receiverdtls->candidate->firstName.' '.$vs->receiverdtls->candidate->lastName;
                    }
                    //$name          = ($vs->receiverdtls->candidate->firstName)?$vs->receiverdtls->candidate->firstName:'';
                  }
                  //$name = ($vs->receiverdtls->candidate->firstName)?$vs->receiverdtls->candidate->firstName:'';
                }
                $dataArr     = array('msgId' => $vs['msgId'],'convoHead' => $vs['convoHead'],'msgFrom' => $vs['msgFrom'],'msgTo' => $vs['msgTo'],'msgText' => $vs['msgText'],'msgFileName' => $vs['msgOriginalFileName'],'msgFile' => $vs['msgFile'],'readStatus' => $vs['readStatus'],'createdOn' => $vs['createdOn'],'candidateName' => $name, 'companyName' => session('employer_session_data.employerCompany'));
