...
Description   : Controller file for managing Partner
...
class LoginController extends AppController {
  ...
  public function register(){
    ...
    if(!empty(request()->all()) && request()->isMethod('post')) {
      ...
      $validator   =  Validator::make($requestData, [
        ...
        'organizationName'     => 'bail|required|regex:/^[a-zA-Z0-9_\- ]*$/|max:128',
        'emailId'         => 'bail|required|email|max:128',
        'phoneNo'         => 'bail|required|numeric',
        'passwordpartnerfield'        => 'bail|required|required_with:cpasswordpartner|same:cpasswordpartner',
        'cpasswordpartner'       => 'bail|required',
        'acceptpartner'  => 'bail|required|numeric'
      ], [
        'fullName.required'         => 'Full name is required',
        'fullName.regex'            => 'Full name should be alphabetic',
        'fullName.max'              => 'Full name should be 128 character',
        'organizationName.required'      => 'Company name is required',
        'organizationName.regex'         => 'Company name should be alphabetic',
        'organizationName.max'           => 'Company name should be 128 character',
        ...
        'passwordpartnerfield.required'         => 'Password is required',
        'passwordpartnerfield.same'             => 'The password and confirm password must match',
        'cpasswordpartner.required'        => 'Confirm password is required',
        'acceptpartner.required'   => 'Please accept the terms and condition',
        'acceptpartner.numeric'   => 'Please accept the terms and condition'
      ]);

      if($validator->fails()){
        return redirect('ngo/register')->withErrors($validator)->withInput();
      }else{
        /*************************google recaptcha validation by :: samir kumar**********************************/
         /* $captcha = $requestData['g-recaptcha-response'];
          ...
          //  $chkDup                        = AdminModel::where([['emailId',$requestData['emailId']],['tinUserType',4]])->orWhere('loginId', $requestData['emailId'])->orWhere('mobileNo', $requestData['phoneNo'])->first();
            $chkDup                        = AdminModel::where([['emailId',$requestData['emailId']],['tinUserType',4]])->orWhere('loginId', $requestData['emailId'])->first();
           ...
           // if(!empty($chkDup) && $chkDup->mobileVerifyFlag==0){
            if(!empty($chkDup) && $chkDup->emailVerifyFlag==0){
                    if($requestData['emailId'] == $chkDup->emailId && $requestData['phoneNo'] == $chkDup->mobileNo){
                        ...
                        $ccmailAddress='';
                        $bccmailAddress='';
                        $subject='Registration for Partner';
                        $attachment='';
                        ...
                        return redirect('ngo/verifyOtp/'.encrypt($chkDup->userId));
                    }else{
                       $status['status'] = 'error';
                       $status['error_msg'] = "Email already registered! Please use a different email id.";
                       return redirect('ngo/register')->with('error', $status['error_msg'])->withInput();
                    }
           ...
           // }else if(!empty($chkDup) && $chkDup->mobileVerifyFlag==1){
              ...
              $adminModel->companyName       = $requestData['organizationName'];
              $adminModel->emailId           = $requestData['emailId'];
              $adminModel->loginId           = $requestData['emailId'];
              $adminModel->mobileNo          = $requestData['phoneNo'];
              $adminModel->password          = md5($requestData['passwordpartnerfield']);
              $adminModel->tinUserType       = 4;
              $adminModel->otp               = $otp;
              ...
              if($adminModel->save()){
                ...
                $ccmailAddress='';
                $bccmailAddress='';
                $subject='Registration for NGO';
                $attachment='';
                $sendTo= $requestData['emailId'];

                // sendMail($mailContent,$ccmailAddress,$bccmailAddress,$subject,$attachment,$sendTo);
                request()->session()->flash('success', "One Time Password(OTP) has been sent to your Email, please enter the same here to register.<br> If you dont find the mail in your inbox then please check your spam folder.");
                  return redirect('ngo/verifyOtp/'.encrypt($lastInsertedId));
              }else{
    ...
    return view('partner.register',$this->viewVars);
  }
  public function forgetpassword(){
    ...
    if(!empty(request()->all()) && request()->isMethod('post')) {
     ...
     // 'captcha.required'          => 'Captcha is required',
      ...
      if ($validator->fails()) {
        return redirect('ngo/forgetpassword')->withErrors($validator)->withInput();
      } else {
        // $image = new Securimage();
        // if($image->check($requestData['captcha']) !== true) {
        //   $status['status'] = 'error';
        //   $status['error_msg'] = "Invalid Captcha Code";
        //   return redirect('ngo/forgetpassword')->with('error', $status['error_msg']);
        // }else{
          $userName=$requestData['emailId'];
          $getPartnerData = $adminModel->where([['loginId',$userName],['tinUserType',4],['publishStatus',1]])->get()->toArray();
          if(!empty($getPartnerData)){
            $encryptedurl = encrypt($getPartnerData[0]['userId'].'~::~'.time());
           $varurl='<a href='.ROOT_URL.'/employer/resetpassword/'.$encryptedurl.'> click here</a>';
            $mailContent='click on this link for Account Verification '. $varurl;
                ...
                $attachment='';
                $sendTo=$getPartnerData[0]['loginId'];
   ...
   $bannerList=BannerModel::where([['deletedflag',0],['publishStatus',0],['pageType',$pageType]])->orderBy('bannerId','ASC')->get();
   $this->viewVars['banners']  = $bannerList;   
   return view('partner.forgetpassword',$this->viewVars);
 }
 ...
 public function resetpassword($url){
  ...
  if($hour>=1){
   ...
   return redirect('ngo/forgetpassword');
 }
 if(!empty(request()->all()) && request()->isMethod('post')) {
 ...
 // 'captcha.required'          => 'Captcha is required',      
  ...
  if ($validator->fails()) {
    return redirect('ngo/resetpassword/'.$url)->withErrors($validator)->withInput();
  } else {
  //  $image = new Securimage();
  //  if($image->check($requestData['captcha']) !== true) {
  //   $status['status'] = 'error';
  //   $status['error_msg'] = "Invalid Captcha Code";
  //   return redirect('ngo/resetpassword/'.$url)->with('error', $status['error_msg']);
  // }else{
...
public function changepassword()
{
    ...
    if(!empty(request()->all()) && request()->isMethod('post')) {
      ...
      if ($validator->fails()) {
        return redirect('ngo/changepassword/')->withErrors($validator)->withInput();
      }else{
        if(md5($requestData['oldpassword'])==md5($requestData['newpassword'])){
         ...
         return redirect('ngo/changepassword/')->with('error', $status['error_msg']);
       }
       else{
        $userid=session('partner_session_data.userId');
        $getPartnerData = $adminModel->where([['userId',$userid],['tinUserType',4]])->get()->toArray();
        if(!empty($getPartnerData) && ($getPartnerData[0]['password']==md5($requestData['oldpassword']))){
          $updatedata=AdminModel::where('userId', $userid)
          ->update([
            'password' => md5($requestData['newpassword'])
          ]);
...
public function verifyOtp($encid){
  ...
  $adminModel = new AdminModel();
  $getPartnerData = $adminModel->where([['userId',$userid]])->get()->toArray();
   $this->viewVars['getPartnerData']=$getPartnerData;
    if($getPartnerData[0]['emailVerifyFlag']==1)
   {
  ...
  $data=request()->all();
    if(!empty(request()->all()) && request()->isMethod('post')) {
      $requestData = request()->all();
      $validator   =  Validator::make($requestData, [
       'otppartner'         => 'bail|required',
     ], [
      'otppartner.required'          => 'otp is required',      
    ]);
      if ($validator->fails()) {
        return redirect('ngo/verifyOtp/'.$encid)->withErrors($validator)->withInput();
      }else{
     
          $updatedata=AdminModel::where([['userId',$userid],['otp',$data['otppartner']]])
          ->update([
            'publishStatus' => 1,
            'emailVerifyFlag'=>1
          ]);
       ...
       if(!empty($updatedata)){
         ...
         return redirect('ngo/verifyOtp/'.$encid)->with('error', $status['error_msg']);
           //request()->session()->flash('error', 'The OTP entered is incorrect.');
          }
     }
  }

 return view('partner.verifyOtp', $this->viewVars);
 }
}
